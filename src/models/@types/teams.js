/** @typedef {Object} TeamModel
 * @property {number} year - The year the team was established.
 * @property {string} pcsId - The unique identifier for the team.
 * @property {string} name - The name of the team.
 * @property {string} classification - The classification of the team.
 * @property {string} jerseyImagePcsUrl - The URL of the team's jersey image.
 * @property {string} previousPcsId - The unique identifier of the previous team.
 * @property {string} nextPcsId - The unique identifier of the next team.
 */

export {};
